...
- **Endpoints:**
  ...
  - `POST /api/jira/sync` — body `{ "snapshot": "<full export JSON string>", "teamId": "<id>" }`; optional `"jql": "..."` overrides team JQL; optional `"syncMode": "admin" | "individual"` and `"trackerUsername": "<login>"` — **admin** uses the team PAT and team JQL only; **individual** uses that user’s PAT, the same team JQL, plus Jira issues where `reporter = currentUser()` and `created` falls in the tracker’s **current calendar sprint** window. Items in that second set that are not on a Jira sprint matching the active tracker sprint get `jiraNeedsSprintLabel` in the returned snapshot for admins. **Response** includes `commentFetchFailureCount` (number of issues where `GET /rest/api/2/issue/{key}/comment` failed); the server keeps existing `jira-cmt-*` mirror rows for those issues when present, and logs `[jira sync] comments fetch failed for …` to stderr.
  - `POST /api/jira/issue-comment` — body `{ "teamId": "<id>", "issueKey": "PROJ-123", "body": "<plain text>" }`; optional `"syncMode": "admin" | "individual"` and `"trackerUsername"` (same rules as sync). Posts a comment on Jira using the configured PAT. Response `{ "ok": true, "jiraCommentId": "<id>" }` when Jira returns an id (the client may rewrite the local comment id to `jira-cmt-<id>` so the next sync does not duplicate). The **web UI** sends the same text as in the tracker (no attribution prefix); users opt in via **“Also post to Jira”** and pick the linked issue when several exist.
  - `PUT /api/jira/issue-comment` — body `{ "teamId": "<id>", "issueKey": "PROJ-123", "jiraCommentId": "<numeric id>", "body": "<plain text>" }`; same optional `syncMode` / `trackerUsername`. Updates an existing Jira comment (used when the user edits a `jira-cmt-*` row and opts to **sync the edit to Jira**). Response `{ "ok": true }` on success.
